...
class Staff::VisitorsStep
  include MemoryModel

  attribute :email_address, :string
  attribute :phone_no, :string
  attribute :visitors, :pvbcommon_visitor_list
  ...
  def backfilled_visitors
    existing = visitors
    num_needed = Staff::Prison::MAX_VISITORS - existing.count
    backfill = num_needed.times.map { Staff::Visitor.new }
  ...
  def visitors_attributes=(params)
    ...
    self.visitors = pruned.empty? ? [{}] : pruned.take(Staff::Prison::MAX_VISITORS)
  end
