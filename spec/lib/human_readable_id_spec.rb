...
RSpec.describe HumanReadableId do
  describe '.update_unique_id' do
    subject { described_class.update_unique_id(Staff::Visit, visit.id, :human_id) }

    let(:visit) { FactoryBot.create(:staff_visit, human_id: nil) }
    ...
    context 'when an id has already been saved to the DB' do
      before do
        described_class.update_unique_id(Staff::Visit, visit.id, :human_id)
      end
    ...
    context 'when a duplicate id is generated' do
      let(:other_visit) { FactoryBot.create(:staff_visit, human_id: nil) }

      before do
        described_class.update_unique_id(Staff::Visit, other_visit.id, :human_id)
    ...
    context 'when human_readable?' do
      it '#human_readable?' do
        expect(described_class).to be_human_readable('')
      end
    end
  end
end
