...
RSpec.describe Vsip::Api do
  ...
  before do
    Rails.configuration.sentry_dsn_previous = Rails.configuration.sentry_dsn
    Rails.configuration.sentry_dsn = nil
    Rails.configuration.vsip_host = 'http://example.com'
  end

  after do
    Rails.configuration.sentry_dsn = Rails.configuration.sentry_dsn_previous
  end
  ...
  describe 'visit_sessions' do
    let(:session_date) { Time.zone.today.to_s }
    ...
    context 'when there is an api error' do
      ...
      it 'retrieves VSiP sessions' do
        expect(described_class.instance.visit_sessions(1, 1)).to eq({ vsip_api_failed: true })
      end
    end
  end
end
