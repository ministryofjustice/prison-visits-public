...
RSpec.describe Staff::Visit, type: :model do
  include StaffResponseHelper
  subject { build(:staff_visit, visitors: [build(:staff_visitor)]) }
  ...
  describe 'state' do
    ...
    context 'when .visit_state_changes' do
      it { is_expected.to have_many(:visit_state_changes) }
  ...
  describe 'confirm_by' do
    let(:prison) { instance_double(Staff::Prison) }
  ...
  describe '#allowed_additional_visitors' do
    let(:visitor1) { FactoryBot.build_stubbed(:staff_visitor) }
    let(:visitor2) { FactoryBot.build_stubbed(:staff_visitor) }
    let(:visitor3) { FactoryBot.build_stubbed(:staff_visitor, banned: true) }
